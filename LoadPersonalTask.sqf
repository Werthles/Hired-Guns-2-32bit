...
//		+ (HGDifficulty*AddedDifficulty);
		...
		for [{_i=0}, {_i<_loop}, {_i=_i+1}] do
		{
			...
			_newGroup = grpNull;
			//_newGroup = [_Position, EAST, _unitArray, _relativeArray,[],_skill,[],[],_Azimuth] call BIS_fnc_spawnGroup;
			_newGroup = [_wayPos, EAST, _unitArray, [], _rankArray, _skill, _ammo, _unitNumber] call BIS_fnc_spawnGroup;
			sleep 1;
 ...
 */
	[_name,_caller, _theGroup, _missionType, _loot, _missionFileName, _missionLevel, _newTask, _groupArray, _title] execVM "LoadPersonalTaskEnd.sqf";
