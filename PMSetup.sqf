...
PersonalMissionArray = []; //list of missions
{
	...
	_typeLocations = _x;
	{
		...
		_nextMission set [3, (PMDescriptions select _i) +"<br/>Hiring Cost: T$ " + (str (PMMoneyArray select _j)) + ",000 Per Member<br/>Reward: " + (PMRewardArray select _i select _j) + "<br/>Location: " + format["%1",(text ((nearestLocations [_x,["NameCityCapital","NameCity","NameVillage"],2000]) select 0))]+"<br/>The job leader must collect the target themselves, then take them it the assigned cache to complete the job. This will then distribute cash/loot into player caches."]; //short description
